      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <h3 className="mt-1.5 text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">{copy.role}</h3>
          </div>
        </div>
        <div className="shrink-0 lg:text-right">
          <p className="font-mono text-sm font-medium text-gray-500">{copy.period}</p>
          <p className="mono-label mt-1">{copy.location}</p>
        </div>
      </div>

      <p className="mt-5 max-w-3xl text-base leading-relaxed text-gray-600">{copy.summary}</p>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1.25fr_0.75fr]">
        <ul className="flex flex-col">
          {copy.achievements.slice(0, 4).map((achievement, i) => (
            <li
              key={achievement}
              className="flex gap-5 border-b border-gray-100 py-3.5"
            >
              <span className="shrink-0 font-mono text-[11px] text-volt">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="text-[15px] leading-snug text-gray-700">{achievement}</span>
            </li>
          ))}
        </ul>
        ...
        <div>
          ...
          <ul className="flex flex-wrap gap-1.5">
            {experience.stack.map((tech) => (
              <li
                key={tech}
                className="flex items-center gap-1.5 border border-gray-200 px-2.5 py-1.5 text-[12px] text-gray-600 transition-colors hover:border-volt/40 hover:text-gray-900"
              >
            ...
            <>
              ...
              <dl className="grid grid-cols-2 gap-px bg-gray-100">
                {experience.metrics.map((metric) => (
                  <div key={metric.key} className="bg-gray-50 p-4">
