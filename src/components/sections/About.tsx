  ...
  return (
    <div ref={ref} className="shell py-20 sm:py-24">
      ...
      <motion.dl
        ...
        className="mt-14 grid border-t border-white/10 sm:grid-cols-2 lg:grid-cols-3"
      >
        {FACT_KEYS.map((key) => (
          <div
            key={key}
            className="flex flex-col gap-1 border-b border-white/8 py-4 pr-6 sm:odd:border-r lg:odd:border-r-0 lg:[&:not(:nth-child(3n))]:border-r"
          >
      ...
      >
        ...
        <ol className="grid gap-px border border-white/10 bg-white/10 sm:grid-cols-2 lg:grid-cols-4">
          {PRINCIPLE_KEYS.map((key, i) => (
            <li key={key} className="group bg-ink p-5 transition-colors hover:bg-ink-2">
              <div className="flex items-start justify-between gap-4">
                ...
                <UiIcon
                  ...
                  className="size-4 text-white/30 transition-colors group-hover:text-volt"
                  aria-hidden
                />
              </div>
